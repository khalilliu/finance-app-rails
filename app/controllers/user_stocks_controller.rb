class UserStocksController < ApplicationController
    ...
    if params[:stock_id].present?
      ...
      if stock
        ...
        if stock.save
          ...
          @user_stock = nil
          flash[:error] = "Stock is not avaliable"
        end
      end
    end
    ...
    respond_to do |format|
      if @user_stock.save
        format.html { redirect_to my_portfolio_path, 
          notice: "Stock #{@user_stock.stock.ticker} stocks was successfully added" }
  ...
  private
    ...
    def set_user_stock
      @user_stock =  UserStock.find_by_stock_id_and_user_id(params[:id],current_user.id)
    end
